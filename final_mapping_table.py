...
def create_comprehensive_mapping_table():
    ...
    # Write to file
    with open('comprehensive_question_mapping.md', 'w', encoding='utf-8') as f:
        f.write("| PDF Question | App Question | PDF Answer | App Answer | PDF Question Text | App Question Text |\n")
        f.write("|-------------|--------------|------------|------------|-------------------|-------------------|\n")
        ...
        for i in range(1, max_questions + 1):
            ...
            pdf_question_text = pdf_question_text.replace('|', '\\|').replace('\n', ' ')
            app_question_text = app_question_text.replace('|', '\\|').replace('\n', ' ')

            f.write(f"| {pdf_q_num} | {app_q_num} | {pdf_answer} | {app_answer} | {pdf_question_text} | {app_question_text} |\n")

    print("Comprehensive mapping table saved to: comprehensive_question_mapping.md")
